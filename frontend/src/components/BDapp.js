...
const BDapp = () => {
    ...
    // data recieved from db - init: undefined
    const [userTickets, setUserTickets] = useState(undefined);
    const [stopFetch, setStopFetch] = useState(false);

    useEffect(() => {

        if (selectedAddress && balance && !userTickets && !stopFetch) {

            fetch('http://127.0.0.1:3001/getUserTickets', {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
                body: JSON.stringify({ address: selectedAddress }) // CHANGE THESE
            })
            .then(response => {
                if (!response.ok) {                                  // ***
                ...
                .catch(error => {
                    ...
                    setUserTickets(false)
                // ...handle/report error
                })
                .then((jsonRes) => {
    
                    setUserTickets(jsonRes)
    ...
    }, [selectedAddress, balance, stopFetch, userTickets])
    ...
    const getTokenData = async () => {
        ...
        const name = await tokenE.name();
        const symbol = await tokenE.symbol();
    ...
    const updateBalance = async () => {
        ...
        let balanceE = await tokenE.balanceOf(selectedAddress);
        setBalance(balanceE);

        // ISSUE HERE, CONSTANT RELAODING STATE
    }
    ...
    const transferTokens = async (to, amount) => {
        ...
        const tx = await tokenE.transfer(to, amount);
        ...
        return(
            ...
            <div className="ticket-page">
                ...
                <div className="nav">
                    ...
                    <div className="unselect"><div><span style={{fontWeight: 450}}>ticket.</span>decentral<span style={{fontWeight: 450}}>*</span></div><div className="sub unselect"><span style={{fontWeight: 450}}>*</span>own your events</div></div>
                    <div></div>
